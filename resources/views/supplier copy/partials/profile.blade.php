...
@php
    ...
    $hasOverview = filled($profile?->manufacturing_description);

    $hasStatistics =
        filled($profile?->factory_area) ||
        filled($profile?->production_lines) ||
        filled($profile?->moq) ||
        filled($profile?->monthly_capacity) ||
        filled($profile?->lead_time_days);

    $hasManufacturingBlock =
        $hasCapabilities ||
        $hasOverview ||
        $hasStatistics;
@endphp

@if($hasManufacturingBlock)
    ...
    <div class="p-6 space-y-8">
        ...
        @if($hasOverview)
            ...
            <div>
                ...
                <div class="text-sm leading-7 text-gray-700">
                    {!! $profile->manufacturing_description !!}
                </div>
        ...
        @if($hasStatistics)
            ...
            <div class="grid grid-cols-2 lg:grid-cols-5 gap-4">
                ...
                @if(filled($profile?->factory_area))
                    <div class="rounded-xl border border-gray-200 p-5">
                        <div class="text-xs uppercase tracking-wide text-gray-400">
                            Facility Area
                        </div>
                        ...
                        <div class="mt-2 text-lg font-bold text-gray-900">
                            {{ $profile->factory_area }} m²
                        </div>
                    </div>
                @endif


                @if(filled($profile?->production_lines))
                    <div class="rounded-xl border border-gray-200 p-5">
                        <div class="text-xs uppercase tracking-wide text-gray-400">
                            Production Lines
                        </div>
                ...
                @if(filled($profile?->monthly_capacity))
                    <div class="rounded-xl border border-gray-200 p-5">
                        <div class="text-xs uppercase tracking-wide text-gray-400">
                            Monthly Capacity
                        </div>
                ...
                @if(filled($profile?->lead_time_days))
                    <div class="rounded-xl border border-gray-200 p-5">
                        <div class="text-xs uppercase tracking-wide text-gray-400">
                            Lead Time
                        </div>
