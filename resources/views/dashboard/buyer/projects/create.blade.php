...
<div class="max-w-3xl mx-auto bg-white border rounded-xl shadow-sm p-6">
    ...
    <div class="mb-6">
        ...
        <p class="text-sm text-gray-500">
            Add a new project. You can later add items, materials, descriptions, and colors.
        </p>
    </div>
    ...
    <form action="{{ route('buyer.projects.store') }}" method="POST" class="space-y-5">
        @csrf
        ...
        <div>
            ...
            <textarea name="description" rows="5"
                      class="w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
                      placeholder="Optional: add a general note or description for this project">{{ old('description') }}</textarea>
            @error('description')
            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            @enderror
        </div>
