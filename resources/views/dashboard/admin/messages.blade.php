...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-6" style="height: calc(60vh);">
    ...
    <div class="lg:col-span-1 border rounded-lg overflow-y-auto" style="height: 100%;">
        @include('dashboard.admin.partials.message-list', ['threads' => $threads])
    </div>
    ...
    <div class="lg:col-span-2 border rounded-lg flex flex-col overflow-y-auto" style="height: 100%;">
        ...
        <div class="flex-1 overflow-y-auto p-4 space-y-4">
            <div id="chatMessages">
                ...
                @if($product)
                <div class="p-4">
                    @include('dashboard.admin.partials.product-card', ['product' => $product])
                </div>
                @endif


                @include('dashboard.admin.partials.message-thread', ['messages' => $messages])
            </div>
        </div>

        {{-- Form --}}
        <div class="border-t p-4">
            @include('dashboard.admin.partials.message-form', [
...
<script>
    ...
    const addMessageElem = (message, chatMessagesElem) => {
        ...
        chatMessagesElem.insertAdjacentHTML(
            'beforeend',
            message.role === 'admin' ? buyerMsg : sellerMsg
        );
    ...
    const loadThread = async (threadId) => {
        ...
        const res = await fetch(`/dashboard/admin/messages/${threadId}`);
        if (!res.ok) return;
    ...
    const pollMessages = async () => {
        ...
        const res = await fetch(`/dashboard/admin/messages/${activeThreadId}/poll?lastMessage=${lastMessageId}`);
        if (!res.ok) return;
    ...
    document.getElementById('chatForm')?.addEventListener('submit', async e => {
        ...
        const res = await fetch(`/dashboard/admin/messages/${activeThreadId}/send`, {
            method: 'POST',
            headers: {
    ...
    document.querySelectorAll('[data-thread-id]').forEach(li => {
        li.addEventListener('click', async () => {
            window.location = '/dashboard/admin/messages?thread_id=' + li.dataset.threadId;
