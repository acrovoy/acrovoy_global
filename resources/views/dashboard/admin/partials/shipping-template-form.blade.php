...
<form method="POST"
      action="{{ $shippingTemplate && $shippingTemplate->id
                  ? route('admin.shipping-templates.update', $shippingTemplate->id)
                  : route('admin.shipping-templates.store') }}"
    ...
    <input type="hidden" name="logistic_company_id" value="{{ auth()->id() }}">
...
<div class="mb-4">
    ...
    <select name="price_unit" class="input">
        ...
        @foreach($units as $key => $label)
            <option value="{{ $key }}"
                {{ old('price_unit', $shippingTemplate->price_unit ?? '') === $key ? 'selected' : '' }}>
                {{ $label }}
            </option>
        @endforeach
    </select>
    <p class="text-sm text-gray-500 mt-1">
        Select the unit for the price.
    </p>
</div>
