...
<div class="flex flex-col gap-6">
    ...
    <thead class="bg-gray-50 text-gray-600">
        <tr>
            ...
            <th class="px-5 py-3 text-left font-medium">Dispute</th>
    ...
    @forelse($orders as $order)
        ...
        >
            ...
            <td class="px-4 py-2">
                ...
                @endphp
                <span class="px-2 py-1 text-xs font-medium inline-block {{ $statusClasses[$order['status']] ?? 'bg-gray-200 text-gray-600' }}">
    ...
    <ul class="list-none space-y-0 max-h-40 text-xs text-gray-500">
        @foreach($order['items'] as $item)
            <li class="relative group max-w-[180px]">
                
                <span class="block truncate whitespace-nowrap overflow-hidden">
                    {{ $item['product'] }} ({{ $item['qty'] }} pcs)
                </span>
        ...
        @endforeach
            ...
            <td class="px-5 py-3">
                ...
                @if(!empty($order['delivery_price']) && $order['delivery_price'] > 0)
                    <div class="text-xs text-gray-400">(${{ $order['delivery_price'] }})</div>
                @endif
            </td>
            ...
            <td class="px-5 py-3 text-gray-600">{{ $order['customer'] }}</td>

            <td class="px-5 py-3 text-gray-600 text-xs">{{ $order['date'] }}</td>
