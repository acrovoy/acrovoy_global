...
<div class="flex flex-col gap-4">
    ...
    {{-- Notification --}}
    @include('dashboard.supplier.partials.ship-notification', ['$order' => $order, '$r_order' => $r_order])
    ...
    <div class="border rounded-lg p-4 bg-gray-50">
        ...
        @if(count($available) > 0)
            <form method="POST"
                action="{{ route('supplier.orders.update-status', $order['id']) }}"
    ...
    {{-- Product sector --}}
    @include('dashboard.supplier.partials.product-order', ['order_items' => $order_items, 'order' => $order])
   
    @if($order['provider_type'] === \App\Models\LogisticCompany::class)
    @elseif($order['provider_type'] === \App\Models\Supplier::class)
        {{-- shipment-desk --}}
        @include('dashboard.supplier.partials.shipment-desk', ['order_items' => $order_items, 'order' => $order])
    @endif

    {{-- Disputes --}}
    @include('dashboard.supplier.partials.dispute', ['order_items' => $order_items, 'order' => $order])

    {{-- Order status timeline --}}
    @include('dashboard.supplier.partials.status-timeline', ['order_items' => $order_items, 'order' => $order])
...
function pickupModal() {
    return {
        ...
        async fetchRegions() {       
            ...
            try {
                const res = await fetch(`{{ route('supplier.locations.regions') }}?country_id=${this.origin_country_id}`);
        ...
        async fetchCities() {
            ...
            try {
                const res = await fetch(`{{ route('supplier.locations.locations') }}?region_id=${this.origin_region_id}`);
