...
<div class="mb-6 relative">
    ...
    <button
        ...
        {{-- ICON --}}
        <svg class="w-4 h-4 text-gray-500 transition-all duration-300 ease-out group-hover:rotate-180 group-hover:text-gray-700"
             viewBox="0 0 20 20"
             fill="currentColor">
    ...
    <div id="companyDropdown"
        ...
        <form method="POST" action="{{ route('company.switch') }}">
            @csrf
            <input type="hidden" name="company_key" value="personal_buyer|0">
        ...
        <form method="POST" action="{{ route('company.switch') }}">
            @csrf
            <input type="hidden" name="company_key" value="personal_supplier|0">
...
@foreach($menu as $item)
    ...
    @if(($item['type'] ?? null) === 'header')
        ...
        <li class="mt-5 mb-2">
            <div class="px-3 py-2 rounded-lg bg-gray-50/60 border border-gray-100 backdrop-blur-sm">
        ...
        <li>
            <a href="{{ isset($item['route']) ? route($item['route']) : '#' }}"
               class="group flex items-center justify-between px-3 py-2 rounded-lg
                ...
                <span class="absolute left-0 top-1/2 -translate-y-1/2 h-0 w-0.5 bg-brown-500
                ...
                @if(!empty($item['badge']))
                    <span class="ml-2 px-2 p-0.5 text-[11px] font-semibold
